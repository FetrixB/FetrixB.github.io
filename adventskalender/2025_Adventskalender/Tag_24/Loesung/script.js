...
let player = {
    ...
    onGround: true,
    groundHeight: 0,
    maxStepHeight: 2.5, // Maximale Höhe die der Spieler noch erklimmen kann
    playerHeight: 1.8   // Spieler-Größe (Abstand von Füßen zu Augen)
};
 ...
 */
function createSingleGift(x, z) {
    // Ermittele die korrekte Bodenhöhe an dieser Position
    const groundHeight = getGroundHeight(x, z);
    const y = groundHeight + 1; // Geschenk 1 Einheit über dem Boden
    ...
    const giftMaterial = new THREE.MeshLambertMaterial({ 
        ...
        emissiveIntensity: 0.2 // Heller leuchten für bessere Sichtbarkeit
    });
    ...
    gift.userData = { 
        ...
        collected: false,
        baseY: y // Basis-Höhe für das Schweben
    };
...
/**
 * Ermittelt die Bodenhöhe an einer bestimmten Position mit Raycasting
 */
function getGroundHeight(x, z) {
    if (!gameObjects.terrain) return 0;
    
    // Raycaster von oben nach unten
    const raycaster = new THREE.Raycaster();
    const origin = new THREE.Vector3(x, 100, z); // Starte weit oben
    const direction = new THREE.Vector3(0, -1, 0); // Schaue nach unten
    
    raycaster.set(origin, direction);
    
    // Finde Schnittpunkte mit dem Terrain
    const intersects = raycaster.intersectObject(gameObjects.terrain);
    
    if (intersects.length > 0) {
        return intersects[0].point.y; // Höhe des ersten Schnittpunkts
    }
    
    return 0; // Fallback: Grundebene
}

/**
 * Überprüft ob der Spieler dem Terrain folgen kann (nicht zu steil)
 */
function canWalkOnSlope(currentHeight, targetHeight) {
    const heightDifference = Math.abs(targetHeight - currentHeight);
    return heightDifference <= player.maxStepHeight;
}
...
function updatePlayerMovement(deltaTime) {
    ...
    // Sprung-Mechanik
    if (controls.canJump && player.onGround) {
        player.velocity.y = 0.15; // Sprungkraft
    ...
    if (!player.onGround) {
        player.velocity.y -= 0.01; // Schwerkraft
    }
    
    // Bewegung in Welt-Koordinaten umwandeln
    moveVector.applyQuaternion(camera.quaternion);
    moveVector.y = 0; // Keine Y-Bewegung durch WASD (nur durch Sprung/Schwerkraft)
    
    // Neue Position berechnen
    const newX = camera.position.x + moveVector.x;
    const newZ = camera.position.z + moveVector.z;
    
    // Bodenhöhe an der neuen Position ermitteln
    const groundHeight = getGroundHeight(newX, newZ);
    const targetHeight = groundHeight + player.playerHeight;
    
    // Position aktualisieren
    camera.position.x = newX;
    camera.position.z = newZ;
    
    // Terrain-folgende Y-Position (wenn am Boden)
    if (player.onGround) {
        // Prüfe ob die Steigung begehbar ist
        if (canWalkOnSlope(camera.position.y, targetHeight)) {
            camera.position.y = targetHeight;
            player.groundHeight = groundHeight;
        } else {
            // Zu steiler Hang - Bewegung in X/Z rückgängig machen
            camera.position.x -= moveVector.x;
            camera.position.z -= moveVector.z;
        }
    } else {
        // In der Luft - nur Y-Velocity anwenden
        camera.position.y += player.velocity.y;
    }
    
    // Boden-Kollision für Landung
    if (!player.onGround && camera.position.y <= targetHeight) {
        camera.position.y = targetHeight;
        player.velocity.y = 0;
    ...
    // Verhindere, dass der Spieler unter das Terrain fällt
    const minHeight = groundHeight + player.playerHeight;
...
function updateGifts(deltaTime) {
    gameObjects.gifts.forEach(gift => {
        if (!gift.userData.collected) {
            ...
            // Sanftes Auf-und-Ab schweben basierend auf der Terrain-Höhe
            gift.position.y = gift.userData.baseY + Math.sin(Date.now() * 0.003 + gift.position.x) * 0.3;
        }
    });
}
...
function checkCollisions() {
    ...
    // Geschenke-Kollision (Einsammeln) - größere Kollisionssphäre
    gameObjects.gifts.forEach(gift => {
        if (!gift.userData.collected) {
            const distance = playerPos.distanceTo(gift.position);
            if (distance < 4) { // Vergrößerte Kollisionssphäre
                console.log(`💎 Geschenk gefunden! Distanz: ${distance.toFixed(2)}`);
                collectGift(gift);
            }
        }
    });
...
function resetGameState() {
    ...
    // Spieler-Position zurücksetzen - auf dem Terrain starten
    const startGroundHeight = getGroundHeight(0, 0);
    const startY = startGroundHeight + player.playerHeight;
    ...
    player.velocity.y = 0;
    player.onGround = true;
    player.groundHeight = startGroundHeight;
