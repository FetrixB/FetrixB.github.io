...
const bindBoostButton = (timeline) => {
  ...
  boostButton.addEventListener('click', () => {
    ...
    timeline.playbackRate = turboEnabled ? 1.6 : 1;
    boostButton.classList.toggle('is-active', turboEnabled);
    boostButton.textContent = turboEnabled
      ? '🧊 Turbo ausschalten'
      : '⚡ Turbo-Booster aktivieren';
    status.textContent = turboEnabled
      ? 'Turbo aktiv! Der Pixel-Bär gleitet wie ein Elytra-Flug.'
      : 'Gemütlicher Laufmodus aktiv.';

    if (loopHint) {
      loopHint.textContent = turboEnabled
        ? 'Callback ausgelöst → Timeline-Speed erhöht (Anime.js playbackRate).'
        : 'Turbo aus → Timeline wieder bei 1x Geschwindigkeit.';
    }
  });
};
